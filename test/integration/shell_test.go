...
func TestTerminalOpen(t *testing.T) {
	control(t)
	out := shared.Call(t, "terminal_open", nil)
...
func TestTerminalRun(t *testing.T) {
	control(t)
	shared.Call(t, "terminal_open", nil)
	t.Cleanup(func() { X(t, "pkill -f lxterminal 2>/dev/null || true") })
	time.Sleep(1500 * time.Millisecond)

	marker := "/tmp/it-terminal.txt"
	Sh(t, "rm -f %s", marker)
	shared.Call(t, "terminal_run", map[string]any{
...
func TestTerminalRead(t *testing.T) {
	...
	out := shared.Call(t, "terminal_read", map[string]any{"lines": 40})
...
func openShell(t *testing.T) string {
	t.Helper()
	out := shared.Call(t, "shell_open", map[string]any{"shell": "/bin/bash"})
	id := jsonField(t, out, "id")
	if id == "" {
		t.Fatalf("no shell id in %s", trunc(out, 200))
	}
	t.Cleanup(func() { shared.Call(t, "shell_close", map[string]any{"id": id}) })
	return id
}
...
func TestShellOpen(t *testing.T) {
	...
	out := shared.Call(t, "shell_list", nil)
...
func TestShellExec(t *testing.T) {
	...
	shared.Call(t, "shell_exec", map[string]any{
...
func TestShellInput(t *testing.T) {
	...
	shared.Call(t, "shell_input", map[string]any{
...
func TestShellRead(t *testing.T) {
	...
	out := shared.Call(t, "shell_read", map[string]any{"id": id})
...
func TestShellList(t *testing.T) {
	before := shared.Call(t, "shell_list", nil)
	id := openShell(t)
	after := shared.Call(t, "shell_list", nil)
...
func TestShellClose(t *testing.T) {
	...
	shared.Call(t, "shell_close", map[string]any{"id": id})

	if list := shared.Call(t, "shell_list", nil); strings.Contains(list, id) {
	...
	if again := shared.CallErr(t, "shell_close", map[string]any{"id": id}); again == "" {
...
func sshSession(t *testing.T) string {
	t.Helper()
	ip := peerIP(t)
	out := shared.Call(t, "ssh_connect", map[string]any{
		"host": ip, "user": "peer", "password": "peerpass"})
	id := jsonField(t, out, "id")
	if id == "" {
		t.Fatalf("no session id in %s", trunc(out, 200))
	}
	t.Cleanup(func() { shared.Call(t, "ssh_disconnect", map[string]any{"id": id}) })
	return id
}
...
func TestSSHKeygen(t *testing.T) {
	...
	shared.Call(t, "ssh_keygen", map[string]any{
...
func TestSSHConnect(t *testing.T) {
	...
	if list := shared.Call(t, "ssh_list", nil); !strings.Contains(list, id) {
...
func TestSSHExec(t *testing.T) {
	...
	out := shared.Call(t, "ssh_exec", map[string]any{
...
func TestSSHUpload(t *testing.T) {
	...
	shared.Call(t, "ssh_upload", map[string]any{"id": id, "local": local, "remote": remote})
...
func TestSSHDownload(t *testing.T) {
	...
	shared.Call(t, "ssh_download", map[string]any{"id": id, "remote": remote, "local": local})
...
func TestSSHListRemote(t *testing.T) {
	...
	out := shared.Call(t, "ssh_list_remote", map[string]any{
...
func TestSSHList(t *testing.T) {
	before := shared.Call(t, "ssh_list", nil)
	id := sshSession(t)
	after := shared.Call(t, "ssh_list", nil)
...
func TestSSHTunnelLocal(t *testing.T) {
	id := sshSession(t)
	out := shared.Call(t, "ssh_tunnel_local", map[string]any{
...
func TestSSHTunnelRemote(t *testing.T) {
	id := sshSession(t)
	shared.Call(t, "ssh_tunnel_remote", map[string]any{
...
func TestSSHTunnels(t *testing.T) {
	id := sshSession(t)
	shared.Call(t, "ssh_tunnel_local", map[string]any{
		"id": id, "local_addr": "127.0.0.1:18092", "remote_addr": "127.0.0.1:22"})

	out := shared.Call(t, "ssh_tunnels", map[string]any{"id": id})
...
func TestSSHTunnelClose(t *testing.T) {
	...
	shared.Call(t, "ssh_tunnel_close", map[string]any{"id": id, "tunnel_id": tid})
...
func TestSSHCopyID(t *testing.T) {
	...
	shared.Call(t, "ssh_copy_id", map[string]any{"id": id, "key_path": key + ".pub"})
	...
	shared.Call(t, "ssh_disconnect", map[string]any{"id": keyID})
}
...
func TestSSHDisconnect(t *testing.T) {
	...
	shared.Call(t, "ssh_disconnect", map[string]any{"id": id})

	if list := shared.Call(t, "ssh_list", nil); strings.Contains(list, id) {
		t.Errorf("%s is still listed after disconnecting:\n%s", id, trunc(list, 300))
	}
	// Using it afterwards has to fail rather than quietly reconnect.
	shared.CallErr(t, "ssh_exec", map[string]any{"id": id, "command": "true"})
}

...
func TestMoveWindow(t *testing.T) {
	...
	shared.Call(t, "move_window", map[string]any{"id": id, "x": 300, "y": 260})
...
func TestResizeWindow(t *testing.T) {
	...
	shared.Call(t, "resize_window", map[string]any{"id": id, "width": 600, "height": 380})
...
func TestMinimizeWindow(t *testing.T) {
	...
	shared.Call(t, "minimize_window", map[string]any{"id": id})
...
func TestMaximizeWindow(t *testing.T) {
	...
	shared.Call(t, "maximize_window", map[string]any{"id": id})
...
func TestRestoreWindow(t *testing.T) {
	...
	// nothing at all.
	shared.Call(t, "restore_window", map[string]any{"id": id})
...
func TestFullscreenWindow(t *testing.T) {
	...
	shared.Call(t, "fullscreen_window", map[string]any{"id": id, "action": "add"})
	eventually(t, 6*time.Second, "fullscreen to be set", func() bool {
		return strings.Contains(wmState(t, id), "FULLSCREEN")
	})

	shared.Call(t, "fullscreen_window", map[string]any{"id": id, "action": "remove"})
...
func TestWindowSetState(t *testing.T) {
	...
	shared.Call(t, "window_set_state", map[string]any{
...
func TestActivateWindow(t *testing.T) {
	...
	shared.Call(t, "activate_window", map[string]any{"id": first})
...
func TestCloseWindow(t *testing.T) {
	...
	shared.Call(t, "close_window", map[string]any{"id": id})
...
func TestWindowProperties(t *testing.T) {
	...
	out := shared.Call(t, "window_properties", map[string]any{"id": id})
...
func TestWindowHierarchy(t *testing.T) {
	...
	out := shared.Call(t, "window_hierarchy", nil)
...
func TestSwitchDesktop(t *testing.T) {
	...
	shared.Call(t, "switch_desktop", map[string]any{"desktop": 1})
	...
	shared.Call(t, "switch_desktop", map[string]any{"desktop": 0})
...
func TestSetWindowDesktop(t *testing.T) {
	...
	shared.Call(t, "set_window_desktop", map[string]any{"id": id, "desktop": 1})
...
func TestWaitForWindow(t *testing.T) {
	...
	shared.CallErr(t, "wait_for_window", map[string]any{
...
func TestWaitForIdle(t *testing.T) {
	...
	out := shared.Call(t, "wait_for_idle", map[string]any{
	...
	busy := shared.Call(t, "wait_for_idle", map[string]any{
...
func TestWait(t *testing.T) {
	start := time.Now()
	shared.Call(t, "wait", map[string]any{"ms": 1200})
	waited := time.Since(start)

...
func TestStartRecording(t *testing.T) {
	control(t)
	out := shared.Call(t, "start_recording", map[string]any{
		"container": "mp4", "fps": 15, "audio": false})
	t.Cleanup(func() { shared.Call(t, "stop_recording", nil) })
...
func TestGetRecordingStatus(t *testing.T) {
	...
	idle := shared.Call(t, "get_recording_status", nil)
	if strings.Contains(idle, "\"recording\": true") {
		t.Fatalf("nothing is recording and the status says otherwise: %s", trunc(idle, 200))
	}

	shared.Call(t, "start_recording", map[string]any{"container": "mp4", "fps": 15, "audio": false})
	t.Cleanup(func() { shared.Call(t, "stop_recording", nil) })

	out := shared.Call(t, "get_recording_status", nil)
...
func TestStopRecording(t *testing.T) {
	...
	out := shared.Call(t, "stop_recording", nil)
	path := firstPath(out, ".mp4")
	if path == "" {
		t.Fatalf("no path in %s", trunc(out, 200))
	}
	...
	shared.CallErr(t, "stop_recording", nil)
}
...
func TestListRecordings(t *testing.T) {
	...
	made := firstPath(shared.Call(t, "stop_recording", nil), ".mp4")

	out := shared.Call(t, "list_recordings", nil)
...
func TestStartRestream(t *testing.T) {
	...
	out := shared.Call(t, "start_restream", map[string]any{
		"url": "udp://127.0.0.1:19999", "platform": "udp"})
	t.Cleanup(func() { shared.Call(t, "stop_restream", nil) })
	...
	if strings.Contains(shared.Call(t, "list_restreams", nil), "19999") {
		return
	}
...
func TestListRestreams(t *testing.T) {
	control(t)
	before := shared.Call(t, "list_restreams", nil)
	shared.Call(t, "start_restream", map[string]any{
		"url": "udp://127.0.0.1:19998", "platform": "udp"})
	t.Cleanup(func() { shared.Call(t, "stop_restream", nil) })
	after := shared.Call(t, "list_restreams", nil)
...
func TestStopRestream(t *testing.T) {
	...
	shared.Call(t, "stop_restream", nil)

	if out := shared.Call(t, "list_restreams", nil); strings.Contains(out, "19997") {
...
func TestSetVolume(t *testing.T) {
	...
	for _, want := range []int{40, 75} {
		shared.Call(t, "set_volume", map[string]any{"percent": want})
...
func TestGetAudioState(t *testing.T) {
	...
	out := shared.Call(t, "get_audio_state", nil)
...
func TestSetResolution(t *testing.T) {
	control(t)
	t.Cleanup(func() {
		shared.Call(t, "set_resolution", map[string]any{"width": 1920, "height": 1080})
	})

	shared.Call(t, "set_resolution", map[string]any{"width": 1600, "height": 900})
...
func TestRequestControl(t *testing.T) {
	shared.Call(t, "release_control", nil)
	...
	state := shared.Call(t, "room_state", nil)
...
func TestReleaseControl(t *testing.T) {
	shared.Call(t, "request_control", map[string]any{"timeout_ms": 8000})
	shared.Call(t, "release_control", nil)
	...
	shared.CallErr(t, "type_text", map[string]any{"text": "should be refused"})
}

func TestRoomState(t *testing.T) {
	shared.Call(t, "release_control", nil)
	free := shared.Call(t, "room_state", nil)
	...
	shared.Call(t, "request_control", map[string]any{"timeout_ms": 8000})
	t.Cleanup(func() { shared.Call(t, "release_control", nil) })
	held := shared.Call(t, "room_state", nil)
...
func TestActionLog(t *testing.T) {
	...
	shared.Call(t, "wait", map[string]any{"ms": 37})

	out := shared.Call(t, "action_log", map[string]any{"limit": 10})
	...
	shared.CallErr(t, "read_file", map[string]any{"path": "/definitely/not/a/file"})
	out = shared.Call(t, "action_log", map[string]any{"limit": 5})
...
func TestToolSearch(t *testing.T) {
	...
	out := shared.Call(t, "tool_search", map[string]any{"query": "record the screen to a file", "limit": 10})
	...
	// catalogue.
	empty := shared.Call(t, "tool_search", map[string]any{
...
// gamepadOrSkip skips when there is no uinput device, which is an optional
// capability the daemon degrades without rather than failing.
func gamepadOrSkip(t *testing.T) {
	t.Helper()
	if Sh(t, "test -e /dev/uinput && echo yes") != "yes" {
		t.Skip("no /dev/uinput in this container, so there is no virtual gamepad")
	}
}

func TestGamepadButton(t *testing.T) {
	control(t)
	gamepadOrSkip(t)
	shared.Call(t, "gamepad_button", map[string]any{"index": 0, "down": true})
	shared.Call(t, "gamepad_button", map[string]any{"index": 0, "down": false})

	// The device has to exist in the kernel's input list, which is the only
	// thing observable without a program reading the joystick.
	if !strings.Contains(Sh(t, "cat /proc/bus/input/devices 2>/dev/null"), "sentineldesk") {
		t.Skip("the virtual pad is not registered under a recognisable name")
	}
}

func TestGamepadTap(t *testing.T) {
	control(t)
	gamepadOrSkip(t)
	shared.Call(t, "gamepad_tap", map[string]any{"index": 1, "hold_ms": 40})
}

func TestGamepadAxis(t *testing.T) {
	control(t)
	gamepadOrSkip(t)
	shared.Call(t, "gamepad_axis", map[string]any{"axis": 0, "value": 0.5})
	shared.Call(t, "gamepad_axis", map[string]any{"axis": 0, "value": 0})
}

func TestGamepadState(t *testing.T) {
	control(t)
	gamepadOrSkip(t)
	shared.Call(t, "gamepad_state", map[string]any{
		"buttons": []any{1, 0, 0, 0}, "axes": []any{0.2, 0, 0, 0}})
	shared.Call(t, "gamepad_state", map[string]any{
		"buttons": []any{0, 0, 0, 0}, "axes": []any{0, 0, 0, 0}})
}

// --- packages, services and snapshots ----------------------------------------

func TestSearchPackages(t *testing.T) {
	out := shared.Call(t, "search_packages", map[string]any{"query": "openssh-client"})
...
func TestInstallPackages(t *testing.T) {
	...
	shared.Call(t, "remove_packages", map[string]any{"packages": []any{pkg}, "timeout_ms": 120000})

	shared.Call(t, "install_packages", map[string]any{
...
func TestRemovePackages(t *testing.T) {
	...
	shared.Call(t, "remove_packages", map[string]any{
...
func TestServiceControl(t *testing.T) {
	...
	out := shared.Call(t, "service_control", map[string]any{
	...
	missing := shared.CallErr(t, "service_control", map[string]any{
...
func TestSnapshotCreate(t *testing.T) {
	...
	shared.Call(t, "snapshot_create", map[string]any{
		"name": name, "note": "integration"})
	t.Cleanup(func() { shared.Call(t, "snapshot_delete", map[string]any{"name": name}) })
...
func TestSnapshotList(t *testing.T) {
	name := "it-snap-list"
	shared.Call(t, "snapshot_create", map[string]any{"name": name, "note": "integration"})
	t.Cleanup(func() { shared.Call(t, "snapshot_delete", map[string]any{"name": name}) })

	out := shared.Call(t, "snapshot_list", nil)
...
func TestSnapshotDelete(t *testing.T) {
	...
	shared.Call(t, "snapshot_delete", map[string]any{"name": name})
	...
	shared.CallErr(t, "snapshot_delete", map[string]any{"name": name})
}
...
func TestSnapshotRestore(t *testing.T) {
	...
	shared.CallErr(t, "snapshot_restore", map[string]any{"name": "no-such-snapshot-at-all"})

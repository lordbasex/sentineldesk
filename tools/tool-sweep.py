...
class Sweep:
    ...
    def _record(self, name, description, args, status, out, note):
        summary = " ".join(str(out).split())[:180]
        self.rows.append({
            "tool": name, "description": description, "args": args,
            "status": status, "output": summary, "note": note,
        })
...
def write_report(rows, path, meta):
    ...
    for r in rows:
        ...
        lines.append(
            f"| {BADGE[r['status']]} | `{r['tool']}` | {r['description']} | "
            f"`{args}` | {detail} |")
...
def main():
    ...
    counts, total = write_report(s.rows, out_path, {"when": when, "version": version})

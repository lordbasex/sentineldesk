	...
	return []toolDef{
		// ---------- terminal persistente ----------
		{
			...
			InputSchema: schema(map[string]any{
				...
				"cols":  pInt("terminal width (default 120)"),
				"rows":  pInt("terminal height (default 34)"),
			}),
		},
		{
			Name:        "shell_exec",
			...
			InputSchema: schema(map[string]any{
				...
				"timeout_ms": pInt("max wait (default 20000)"),
				"quiet_ms":   pInt("idle time that means 'finished' (default 400)"),
			}, "id", "command"),
		},
		{
			Name:        "shell_input",
		...
		},
		{
			...
			InputSchema: schema(map[string]any{
				...
				"timeout_sec": pInt("timeout in seconds (default 60)"),
			}, "id", "command"),
		},
		{
			Name:        "ssh_upload",

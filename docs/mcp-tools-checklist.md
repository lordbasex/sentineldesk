...
- [x] `gamepad_button` — press/release a button (A, B, X, Y, LB, …). ✅ exists (Joystick.Apply)
- [x] `gamepad_axis` — move a stick or trigger (axis, value -1..1). ✅ exists (Joystick.Apply)
- [x] `gamepad_state` — a full set of buttons and axes at once. ✅ exists (Joystick.Apply)
- [x] `gamepad_tap` — press + release of a button, for menus. ✅ exists (composable)
...
- **P1 (fluency)**: the rest of mouse/keyboard/windows, `get_active_window`,
  ...
  `get_mouse_position`.
- **P2 (advanced)**: OCR (`find_text`, `read_screen_text`), workspaces, gamepad,
...
- [x] `search_packages` — search the Debian archive; refreshes the index when it is empty.

...
Run on 06 August 2026, 15:36 against `sentineldesk v1.1.9 (2abacb6) · build 20260806-152530`.
...
|---|---|---|---|---|---|
| 1 | ✅ | `get_desktop_info` | Resolution, session and what the desktop is running | `—` | { "display": ":0", "encoder": "auto", "joystick": true, "memory_used": "1759/7833 MB", "recording": false, "resolution": |
| 2 | ✅ | `get_screen_info` | Screen geometry and how many virtual desktops there are | `—` | { "desktops": 4, "display": ":0", "height": 1080, "width": 1920 } |
| 3 | ✅ | `screenshot` | Capture the whole screen as a PNG | `—` | [image saved: /tmp/mcp-screenshot-1786041341770.png] |
| 4 | ✅ | `screenshot_region` | Capture one rectangle instead of the whole screen | `{"x": 0, "y": 0, "width": 320, "height": 200}` | [image saved: /tmp/mcp-screenshot_region-1786041341812.png] |
| 5 | ✅ | `get_pixel_color` | The colour of one pixel, to assert state without an image | `{"x": 10, "y": 10}` | { "b": 214, "g": 207, "hex": "#c3cfd6", "r": 195 } |
| 6 | ✅ | `read_screen_text` | OCR the screen into text | `{"lang": "eng"}` | mel = Pl sentineldesk@1 5bf5ba7 sweep. html - Chromium vy @ sweep.html x + _ ox CG (@File file///tmp/sweep.html x You ar |
...
| 9 | ✅ | `get_active_window` | Which window has focus | `—` | { "id": "0x01a00003", "desktop": 0, "x": 10, "y": 46, "w": 945, "h": 1024, "class": "Chromium", "title": "sweep.html - C |
...
| 13 | ✅ | `is_running` | Whether a named process is alive | `{"name": "Xvfb"}` | { "pids": [ 54 ], "running": true } |
...
| 18 | ✅ | `wait_for_idle` | Wait until the screen stops changing and the CPU settles | `{"timeout_ms": 4000, "quiet_ms": 400, "ignore_cpu": true}` | { "cpu_percent": 100, "idle": false, "reason": "the screen went still but the CPU is still at 100%", "waited_ms": 4322 } |
...
| 20 | ✅ | `action_log` | The audit trail: every call with its connection and result | `{"limit": 3}` | { "count": 3, "entries": [ { "time": "2026-08-06T15:35:45.920-03:00", "tool": "wait", "args": "{\"ms\":50}", "ok": true, |
...
| 37 | ✅ | `window_properties` | The raw X properties of one window | `{"id": "0x0140000c"}` | { "WM_CLASS": [ "xterm", "XTerm" ], "WM_CLIENT_MACHINE": "15bf5ba7aab7", "WM_COMMAND": [ "{ \"/usr/bin/xterm", "-T", "SW |
...
| 39 | ✅ | `move_window` | Move a window to a position | `{"id": "0x0140000c", "x": 120, "y": 120}` | moved |
| 40 | ✅ | `resize_window` | Resize a window | `{"id": "0x0140000c", "width": 640, "height": 400}` | resized |
...
| 51 | ✅ | `terminal_read` | Read back what the terminal shows | `{"lines": 10}` | { "last_command": "echo sweep-terminal-ok", "last_exit_code": 0, "last_succeeded": true, "text": "sentineldesk@15bf5ba7a |
...
| 54 | ✅ | `list_directory` | List a directory | `{"path": "/tmp"}` | [ { "modified": "2026-08-06T15:31:45-03:00", "name": ".X0-lock", "size": 11, "type": "file" }, { "modified": "2026-08-06 |
...
| 57 | ✅ | `browser_tabs` | List the open tabs | `—` | [ { "id": "A636A4028FD77C477D3E169236E35A63", "title": "sweep.html", "url": "file:///tmp/sweep.html" } ] |
...
| # | | Tool | What it does | Arguments sent | Result |
|---|---|---|---|---|---|
| 63 | ✅ | `ui_tree` | Read the desktop as structure rather than pixels | `{"interactive": true, "limit": 40}` | { "count": 312, "elements": [ { "name": "lxpanel", "ref": "0", "role": "application" }, { "center_x": 960, "center_y": 1 |
...
| 67 | ✅ | `ui_diff` | Only what changed in the tree since the last call | `{"reset": true}` | { "baseline": true, "nodes": 358, "note": "first call: the reference snapshot was stored; the next one returns only the  |
...
| # | | Tool | What it does | Arguments sent | Result |
|---|---|---|---|---|---|
| 74 | ✅ | `start_recording` | Record the screen to a file alongside the live stream | `{"container": "mp4", "fps": 15, "audio": false}` | recording to /home/sentineldesk/Recordings/rec-20260806-153604.mp4 |
| 75 | ✅ | `get_recording_status` | Whether a recording is running, and how big it is | `—` | { "container": "mp4", "path": "/home/sentineldesk/Recordings/rec-20260806-153604.mp4", "recording": true, "seconds": 0,  |
| 76 | ✅ | `wait` | Let the recording collect a second of video | `{"ms": 1200}` | waited 1200 ms |
| 77 | ✅ | `stop_recording` | Stop and finalise the file | `—` | { "path": "/home/sentineldesk/Recordings/rec-20260806-153604.mp4", "size_bytes": 560038 } |
...
| # | | Tool | What it does | Arguments sent | Result |
|---|---|---|---|---|---|
| 86 | ✅ | `shell_open` | Start a shell session that survives between calls | `{"shell": "/bin/bash"}` | { "id": "sh2", "shell": "/bin/bash", "user": "" } |
| 87 | ✅ | `shell_exec` | Run a command in that session and wait for it | `{"id": "sh2", "command": "echo sweep-shell-ok", "timeout_ms": 10000}` | { "completed": true, "output": "sweep-shell-ok" } |
| 88 | ✅ | `shell_input` | Send raw keystrokes, without waiting | `{"id": "sh2", "text": "echo second", "enter": true}` | sent 12 bytes |
| 89 | ✅ | `wait` | Give the shell a moment to produce output | `{"ms": 400}` | waited 400 ms |
| 90 | ✅ | `shell_read` | Read and clear what the session has produced | `{"id": "sh2"}` | echo second second sentineldesk@15bf5ba7aab7:/$ |
| 91 | ✅ | `shell_list` | The open shell sessions | `—` | [ { "alive": true, "id": "sh2", "pending": 0, "seconds": 1, "user": "" } ] |
| 92 | ✅ | `shell_close` | End the session | `{"id": "sh2"}` | session closed |
...
| 96 | ✅ | `service_control` | Ask supervisord about the desktop's services | `{"action": "status"}` | { "action": "status", "as_root": true, "exit_code": 3, "service": "all", "stderr": "", "stdout": "at-spi RUNNING pid 57, |
...
| 100 | ✅ | `ssh_connect` | Open an SSH session to a host | `{"host": "127.0.0.1", "user": "sentineldesk", "key_path": "/home/sentineldesk/.ssh/sweep"}` | { "host": "127.0.0.1:22", "id": "ssh2", "user": "sentineldesk" } |
| 101 | ✅ | `ssh_exec` | Run a command on the remote host | `{"id": "ssh2", "command": "echo sweep-ssh-ok", "timeout_sec": 20}` | { "exit_code": 0, "stderr": "", "stdout": "sweep-ssh-ok\n" } |
| 102 | ✅ | `ssh_upload` | Send a file over SFTP | `{"id": "ssh2", "local": "/tmp/sweep.txt", "remote": "/tmp/sweep-up.txt"}` | uploaded 19 bytes to /tmp/sweep-up.txt |
| 103 | ✅ | `ssh_download` | Fetch a file over SFTP | `{"id": "ssh2", "remote": "/tmp/sweep-up.txt", "local": "/tmp/sweep-down.txt"}` | downloaded 19 bytes to /tmp/sweep-down.txt |
| 104 | ✅ | `ssh_list_remote` | List a directory on the remote host | `{"id": "ssh2", "path": "/tmp"}` | [ { "modified": "2026-08-06T15:36:13-03:00", "name": "sweep-down.txt", "size": 19, "type": "file" }, { "modified": "2026 |
| 105 | ✅ | `ssh_tunnel_local` | Forward a local port to the remote side | `{"id": "ssh2", "local_addr": "127.0.0.1:18080", "remote_addr": "127.0.0.1:8080"}` | { "spec": "127.0.0.1:18080 → 127.0.0.1:8080 (via 127.0.0.1:22)", "tunnel_id": "ssh2-l1" } |
| 106 | ✅ | `ssh_tunnels` | The tunnels open on this session | `{"id": "ssh2"}` | [ { "connections": 0, "id": "ssh2-l1", "kind": "local", "spec": "127.0.0.1:18080 → 127.0.0.1:8080 (via 127.0.0.1:22)" }  |
| 107 | ✅ | `ssh_tunnel_remote` | Forward a remote port back to here | `{"id": "ssh2", "remote_addr": "127.0.0.1:18081", "local_addr": "127.0.0.1:8080"}` | { "spec": "127.0.0.1:22:127.0.0.1:18081 → 127.0.0.1:8080 (reverse)", "tunnel_id": "ssh2-r2" } |
| 108 | ✅ | `ssh_tunnel_close` | Close one tunnel by id | `{"id": "ssh2", "tunnel_id": "ssh2-l1"}` | tunnel closed |
| 109 | ✅ | `ssh_copy_id` | Install the public key on the remote host | `{"id": "ssh2", "key_path": "/home/sentineldesk/.ssh/sweep.pub"}` | key installed on sentineldesk@127.0.0.1:22: installed |
| 110 | ✅ | `ssh_disconnect` | End the SSH session | `{"id": "ssh2"}` | SSH session closed |
...
| 112 | ✅ | `snapshot_list` | The snapshots on disk | `—` | { "dir": "/home/sentineldesk/.sentineldesk-snapshots", "snapshots": [ { "created": "2026-08-06T15:36:16-03:00", "name":  |
...
| 119 | ✅ | `open_app_and_wait` | Launch, wait for the window, focus it, in one call | `{"command": "xterm -T SWEEPKILL -e sleep 300", "match": "SWEEPKILL", "timeout_ms": 20000}` | { "opened": true, "waited_ms": 4830, "window": { "id": "0x0140000c", "desktop": 0, "x": 1363, "y": 209, "w": 484, "h": 3 |

...
Run on 06 August 2026, 15:11 against `sentineldesk v1.1.8 (917b8e7) · build 20260806-150457`.
...
|---|---|---|---|---|---|
| 1 | ✅ | `get_desktop_info` | Resolution, session and what the desktop is running | `—` | { "display": ":0", "encoder": "auto", "joystick": true, "memory_used": "1455/7833 MB", "recording": false, "resolution": |
| 2 | ✅ | `get_screen_info` | Screen geometry and how many virtual desktops there are | `—` | { "desktops": 4, "display": ":0", "height": 1080, "width": 1920 } |
| 3 | ✅ | `screenshot` | Capture the whole screen as a PNG | `—` | [image saved: /tmp/mcp-screenshot-1786039863624.png] |
| 4 | ✅ | `screenshot_region` | Capture one rectangle instead of the whole screen | `{"x": 0, "y": 0, "width": 320, "height": 200}` | [image saved: /tmp/mcp-screenshot_region-1786039863675.png] |
| 5 | ✅ | `get_pixel_color` | The colour of one pixel, to assert state without an image | `{"x": 10, "y": 10}` | { "b": 51, "g": 33, "hex": "#1b2133", "r": 27 } |
| 6 | ✅ | `read_screen_text` | OCR the screen into text | `{"lang": "eng"}` | Pry mf Ms NN e. lameocees CM + Ne ve SSH SN MAASASBAQLS A SEY Tg =x < 4 “= os : by P 3 SES . dlovwapy ei Awl ys \ : Mall |
| 7 | ✅ | `find_text` | OCR, but return where a string is so it can be clicked | `{"text": "Files"}` | no match for "files" on screen |
| 8 | ✅ | `get_mouse_position` | Where the pointer is now | `—` | { "x": 10, "y": 10 } |
...
| 12 | ✅ | `list_processes` | Running processes | `{"filter": "python"}` | [ { "command": "/usr/bin/python3 /usr/bin/supervisord -c /etc/supervisor/supervisord.conf", "cpu": "0.5", "mem": "0.3",  |
...
| 18 | ✅ | `wait_for_idle` | Wait until the screen stops changing and the CPU settles | `{"timeout_ms": 4000, "quiet_ms": 400, "ignore_cpu": true}` | { "cpu_percent": 94, "idle": true, "reason": "the screen went still and the CPU settled", "waited_ms": 681 } |
...
| 20 | ✅ | `action_log` | The audit trail: every call with its connection and result | `{"limit": 3}` | { "count": 3, "entries": [ { "time": "2026-08-06T15:11:07.552-03:00", "tool": "wait", "args": "{\"ms\":50}", "ok": true, |
...
| 34 | ✅ | `wait_for_window` | Wait until a window matching a title appears | `{"match": "SWEEPWIN", "timeout_ms": 15000}` | { "class": "xterm.XTerm", "found": true, "id": "0x0140000c", "title": "SWEEPWIN" } |
...
| 36 | ✅ | `activate_window` | Focus and raise a window by id | `{"id": "0x0140000c"}` | activated window 0x0140000c |
| 37 | ✅ | `window_properties` | The raw X properties of one window | `{"id": "0x0140000c"}` | { "WM_CLASS": [ "xterm", "XTerm" ], "WM_CLIENT_MACHINE": "5d39376180ca", "WM_COMMAND": [ "{ \"/usr/bin/xterm", "-T", "SW |
...
| 39 | ✅ | `move_window` | Move a window to a position | `{"id": "0x0140000c", "x": 120, "y": 120}` | moved 0x0140000c (0,120,120,-1,-1) |
| 40 | ✅ | `resize_window` | Resize a window | `{"id": "0x0140000c", "width": 640, "height": 400}` | resized 0x0140000c (0,-1,-1,640,400) |
| 41 | ✅ | `minimize_window` | Minimise a window | `{"id": "0x0140000c"}` | minimized |
| 42 | ✅ | `restore_window` | Restore a minimised window | `{"id": "0x0140000c"}` | restored |
| 43 | ✅ | `maximize_window` | Maximise a window | `{"id": "0x0140000c"}` | maximized |
| 44 | ✅ | `fullscreen_window` | Put a window full screen | `{"id": "0x0140000c"}` | toggled fullscreen |
| 45 | ✅ | `window_set_state` | Change an EWMH state such as 'above' | `{"id": "0x0140000c", "state": "above", "action": "add"}` | add above en 0x0140000c |
| 46 | ✅ | `set_window_desktop` | Send a window to a virtual desktop | `{"id": "0x0140000c", "desktop": 0}` | moved to desktop |
| 47 | ✅ | `switch_desktop` | Switch to another virtual desktop | `{"desktop": 0}` | switched desktop |
| 48 | ✅ | `close_window` | Close the window the sweep opened | `{"id": "0x0140000c"}` | closed window |
...
| 51 | ✅ | `terminal_read` | Read back what the terminal shows | `{"lines": 10}` | { "last_command": "echo sweep-terminal-ok", "last_exit_code": 0, "last_succeeded": true, "text": "sentineldesk@5d3937618 |
...
| 54 | ✅ | `list_directory` | List a directory | `{"path": "/tmp"}` | [ { "modified": "2026-08-06T15:10:33-03:00", "name": ".X0-lock", "size": 11, "type": "file" }, { "modified": "2026-08-06 |
...
| 57 | ✅ | `browser_tabs` | List the open tabs | `—` | [ { "id": "54665F903153973C0C62A604049B9A98", "title": "sweep.html", "url": "file:///tmp/sweep.html" } ] |
...
| # | | Tool | What it does | Arguments sent | Result |
|---|---|---|---|---|---|
| 74 | ✅ | `start_recording` | Record the screen to a file alongside the live stream | `{"container": "mp4", "fps": 15, "audio": false}` | recording to /home/sentineldesk/Recordings/rec-20260806-151120.mp4 |
| 75 | ✅ | `get_recording_status` | Whether a recording is running, and how big it is | `—` | { "container": "mp4", "path": "/home/sentineldesk/Recordings/rec-20260806-151120.mp4", "recording": true, "seconds": 0,  |
| 76 | ✅ | `wait` | Let the recording collect a second of video | `{"ms": 1200}` | waited 1200 ms |
| 77 | ✅ | `stop_recording` | Stop and finalise the file | `—` | { "path": "/home/sentineldesk/Recordings/rec-20260806-151120.mp4", "size_bytes": 600846 } |
...
| 90 | ✅ | `shell_read` | Read and clear what the session has produced | `{"id": "sh1"}` | echo second second sentineldesk@5d39376180ca:/$ |
...
| 104 | ✅ | `ssh_list_remote` | List a directory on the remote host | `{"id": "ssh1", "path": "/tmp"}` | [ { "modified": "2026-08-06T15:11:31-03:00", "name": "sweep-down.txt", "size": 19, "type": "file" }, { "modified": "2026 |
...
| 112 | ✅ | `snapshot_list` | The snapshots on disk | `—` | { "dir": "/home/sentineldesk/.sentineldesk-snapshots", "snapshots": [ { "created": "2026-08-06T15:11:35-03:00", "name":  |
...
| 119 | ✅ | `open_app_and_wait` | Launch, wait for the window, focus it, in one call | `{"command": "xterm -T SWEEPKILL -e sleep 300", "match": "SWEEPKILL", "timeout_ms": 20000}` | { "opened": true, "waited_ms": 2360, "window": { "id": "0x0240000c", "desktop": 0, "x": 1363, "y": 441, "w": 484, "h": 3 |
